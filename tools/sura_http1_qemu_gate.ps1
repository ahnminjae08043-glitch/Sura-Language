...
$source = Join-Path $root "examples/os/http1_qemu_gate.sura"
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
    throw "HTTP/1.1 QEMU source was not found: $source"
}
...
$arguments = @{
    ...
    ExpectedEfiText = "Sura HTTP/1.1 framing test"
    ExpectedMarker = "SURA_HTTP1_FRAMING_OK"
    ExpectedExitCode = 33
}
if ($CompileOnly) {
