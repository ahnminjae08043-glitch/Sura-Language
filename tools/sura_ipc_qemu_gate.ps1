...
$source = Join-Path $root "examples/os/ipc_features.sura"
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
    throw "IPC QEMU source was not found: $source"
}
...
$arguments = @{
    ...
    ExpectedEfiText = "Sura checked IPC event-queue feature test"
    ExpectedMarker = "SURA_IPC_QUEUE_OK"
    ExpectedExitCode = 33
}
if ($CompileOnly) {
