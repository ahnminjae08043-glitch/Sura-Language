...
$source = Join-Path $root "examples/os/css_box_qemu_gate.sura"
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
    throw "CSS box QEMU source was not found: $source"
}
...
$arguments = @{
    ...
    ExpectedEfiText = "Sura CSS box test"
    ExpectedMarker = "SURA_CSS_BOX_OK"
    ExpectedExitCode = 33
}
if ($CompileOnly) {
