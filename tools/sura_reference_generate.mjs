...
const machineFacts = {
  ...
  freestanding: {
    ...
    fast_syscalls_and_user_mode: {
      ...
      limitations: ["entry primitive does not automatically select an address space or validate syscall pointers", "generic interrupt wrapper is not an NMI/MCE paranoid-entry implementation", "no KPTI or comprehensive speculative-entry hardening", "no FPU/SIMD save", "compile and machine-code verification only"],
    },
...
sections.push(section("freestanding", "OS 개발용 freestanding 기능",
  ...
  paragraph("Sura freestanding 기반에는 메모리·페이지 테이블·프로세스·스케줄러·syscall·PCI/PCIe·ACPI·블록 장치·AHCI·NVMe·GPT/MBR·FAT32·VFS·VirtIO-net·Ethernet/DHCP/ARP/IPv4/UDP/DNS/TCP/HTTP·framebuffer·bitmap font·text terminal·window manager·desktop shell·kernel-owned File Explorer/Text Editor/Calculator/Text Browser·CMOS RTC·polling PS/2·serial·QEMU boot gate가 구현되어 있습니다. 현재 QEMU 실행 이미지는 여섯 창, FAT32 하위 디렉터리와 자동 저장·재부팅 복원, DHCP 주소 임대, DNS와 HTTP 응답, 키보드로 입력한 HTTP 주소의 두 번째 탐색, h1/p/a HTML 레이아웃과 CSS 색상 하위 집합, calculator 결과, Start·taskbar activation, HH:MM clock과 active-window routing까지 실행 검증합니다. 아직 없는 핵심 기능은 interrupt-driven input/network queue, resize/minimize/maximize와 Ring 3 application process를 포함한 완전한 interactive window system, FAT32 파일 생성·삭제·크기 변경·긴 이름, IPv6·완전한 TCP 재전송/혼잡 제어·TLS, 중첩 DOM box layout·이미지·폼·일반 CSS cascade/box/flex/grid·JavaScript 브라우저, USB HID, audio, accelerated GPU driver, SMP load balancing, complete per-AP lifecycle, FPU/SIMD process state, ARM64 freestanding backend입니다. 자세한 범위와 각 기능의 compile-only 또는 executed 검증 구분은 " + code("Guide/OS_DEVELOPMENT.md") + "와 이 문서의 machine-readable data에 있습니다.")
));
