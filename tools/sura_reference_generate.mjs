...
const machineFacts = {
  ...
  freestanding: {
    ...
    desktop_shell: {
      ...
      actions: ["Start toggle", "Terminal menu", "System Information menu", "shutdown", "Terminal taskbar", "System Information taskbar", "About icon", "Files icon"],
    ...
    minimal_os_integration: {
      ...
      screenshots: ["build/os/SuraOS-windows.ppm after drag", "build/os/SuraOS-start-menu.ppm with reopened System Information", "build/os/SuraOS-desktop.ppm after Terminal activation"],
      ...
      limitations: ["graphical desktop milestone, not a complete desktop OS", "polling PS/2 and COM1 input rather than interrupt-driven queues", "no resize, minimize, maximize, or application process windows", "fixed kernel-owned Start/taskbar entries", "does not execute every compile-verified freestanding subsystem", "no user program execution, persistent filesystem mutation, network, USB, audio, or browser"],
    },
    acpi_madt: {
...
sections.push(section("freestanding", "OS 개발용 freestanding 기능",
  ...
  paragraph("Sura freestanding 기반에는 메모리·페이지 테이블·프로세스·스케줄러·syscall·PCI/PCIe·ACPI·블록 장치·AHCI·NVMe·GPT/MBR·FAT32·VFS·framebuffer·bitmap font·text terminal·window manager·desktop shell·CMOS RTC·polling PS/2·serial·QEMU boot gate가 구현되어 있습니다. 현재 QEMU 실행 이미지는 그래픽 데스크톱, graphical terminal의 input/output/wrap/scroll/clear, software mouse pointer, 두 창의 focus/z-order/title-bar drag/close/reopen/bounds, Start·taskbar activation, HH:MM clock과 active-terminal keyboard routing까지 실행 검증합니다. 아직 없는 핵심 기능은 interrupt-driven input queue, resize/minimize/maximize와 application process를 포함한 완전한 interactive window system, user application execution, persistent filesystem create/delete, network/browser, USB HID, audio, accelerated GPU driver, SMP load balancing, complete per-AP lifecycle, FPU/SIMD process state, ARM64 freestanding backend입니다. 자세한 범위와 각 기능의 compile-only 또는 executed 검증 구분은 " + code("Guide/OS_DEVELOPMENT.md") + "와 이 문서의 machine-readable data에 있습니다.")
));
