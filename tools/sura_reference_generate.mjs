...
const machineFacts = {
  ...
  freestanding: {
    ...
    desktop_applications: {
      ...
      executed_verification: "QEMU mounts the FAT32 data disk, opens all three apps, appends SURA NOTES and flushes it through AHCI, evaluates 50 - 31 = 19 through the Calculator CPL-3 path, requires app/input/result/storage and SURA_OS_CALCULATOR_RING3_READY/OK markers, and captures build/os/SuraOS-apps.ppm",
      limitations: ["File Explorer, Text Editor, and all window rendering remain kernel-owned", "Calculator uses the kernel page-table root and synchronous CPL-3 entry rather than a separate CR3 or scheduled process", "File Explorer supports bounded short-name directory traversal only", "Text Editor overwrites one fixed 512-byte file", "Calculator is unsigned integer only"],
    },
    cmos_rtc: {
...
sections.push(section("freestanding", "OS 개발용 freestanding 기능",
  ...
  paragraph("Sura freestanding 기반에는 메모리·페이지 테이블·프로세스·스케줄러·syscall·PCI/PCIe·ACPI·블록 장치·AHCI·NVMe·GPT/MBR·FAT32·VFS·VirtIO-net·Ethernet/DHCP/ARP/IPv4/UDP/DNS/TCP/HTTP·framebuffer·bitmap font·text terminal·window manager·desktop shell·File Explorer·Text Editor·Calculator·Text Browser·CMOS RTC·polling PS/2·serial·QEMU boot gate가 구현되어 있습니다. 현재 QEMU 실행 이미지는 여섯 개의 kernel-rendered 창, FAT32 하위 디렉터리와 자동 저장·재부팅 복원, DHCP 주소 임대, DNS와 HTTP 응답, 키보드로 입력한 HTTP 주소의 두 번째 탐색, h1/p/a HTML 레이아웃과 CSS 색상 하위 집합, bounded mailbox를 사용한 Calculator CPL 3 상태 전이, Start·taskbar activation, HH:MM clock과 active-window routing까지 실행 검증합니다. Calculator 경로는 같은 page-table root에서 동기식으로 실행되므로 별도 CR3와 scheduler를 가진 독립 process는 아닙니다. 아직 없는 핵심 기능은 interrupt-driven input/network queue, resize/minimize/maximize와 scheduled Ring 3 application process를 포함한 완전한 interactive window system, FAT32 파일 생성·삭제·크기 변경·긴 이름, IPv6·완전한 TCP 재전송/혼잡 제어·TLS, 중첩 DOM box layout·이미지·폼·일반 CSS cascade/box/flex/grid·JavaScript 브라우저, USB HID, audio, accelerated GPU driver, SMP load balancing, complete per-AP lifecycle, FPU/SIMD process state, ARM64 freestanding backend입니다. 자세한 범위와 각 기능의 compile-only 또는 executed 검증 구분은 " + code("Guide/OS_DEVELOPMENT.md") + "와 이 문서의 machine-readable data에 있습니다.")
));
