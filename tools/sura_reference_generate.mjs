...
const machineFacts = {
  ...
  freestanding: {
    ...
    framebuffer_graphics: {
      ...
      limitations: ["no hardware acceleration or GPU driver", "no USB input", "no interactive windows", "font maps lowercase ASCII to uppercase and has a limited glyph set"],
    },
    ...
    window_manager_foundation: {
      ...
      verification: "compile and EFI image verification; not yet connected to the executed OS desktop",
    ...
    ps2_desktop_input: {
      ...
      executed_verification: "tools/sura_os_screenshot.ps1 exercises Shift, Backspace, status, mouse movement, and a left click through QEMU QMP, then requires the keyboard/Shift/move/click markers and kernel: ready",
      limitations: ["polling rather than IRQ1/IRQ12 queues", "no key repeat policy, layout selection, or Unicode input", "no wheel or five-button packets", "no USB HID", "no focus, drag, resize, or click dispatch"],
    },
    context_primitives: {
      frame_bytes: 72,
    ...
    minimal_os_integration: {
      ...
      limitations: ["graphical desktop milestone, not a complete desktop OS", "polling PS/2 and COM1 input rather than interrupt-driven queues", "mouse pointer without focus, click, drag, resize, or interactive window management", "does not execute every compile-verified freestanding subsystem", "no user program execution, persistent filesystem mutation, network, USB, audio, or browser"],
    },
    acpi_madt: {
...
sections.push(section("freestanding", "OS 개발용 freestanding 기능",
  ...
  table(["영역", "현재 구현"], [
    ...
    ["창 관리자 기반", code("window_manager.sura") + "의 focus, z-order, hit test, title-bar drag, close, screen-bound clamp; compile·EFI image 검증, 실행 OS 연결 전"],
  ...
  paragraph("Sura freestanding 기반에는 메모리·페이지 테이블·프로세스·스케줄러·syscall·PCI/PCIe·ACPI·블록 장치·AHCI·NVMe·GPT/MBR·FAT32·VFS·framebuffer·bitmap font·text terminal·window manager foundation·polling PS/2·serial·QEMU boot gate가 구현되어 있습니다. 현재 QEMU 실행 이미지는 그래픽 데스크톱, graphical terminal의 input/output/wrap/scroll/clear와 software mouse pointer까지 실행 검증합니다. 창 관리자 기반은 focus/z-order/hit test/drag/close/bounds를 compile·EFI image 검증했지만 실행 OS에는 아직 연결하지 않았습니다. 아직 없는 핵심 기능은 interrupt-driven input queue, resize/minimize/maximize와 application process를 포함한 완전한 interactive window system, user application execution, persistent filesystem create/delete, network/browser, USB HID, audio, accelerated GPU driver, SMP load balancing, complete per-AP lifecycle, FPU/SIMD process state, ARM64 freestanding backend입니다. 자세한 범위와 각 기능의 compile-only 또는 executed 검증 구분은 " + code("Guide/OS_DEVELOPMENT.md") + "와 이 문서의 machine-readable data에 있습니다.")
));
