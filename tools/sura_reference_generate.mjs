...
const machineFacts = {
  ...
  freestanding: {
    ...
    control_flow: ["integer arithmetic and comparisons", "if/while/repeat", "break/continue", "and/or short-circuit with selected-operand result"],
    ...
    framebuffer_graphics: {
      libraries: ["stdlib/freestanding/framebuffer.sura", "stdlib/freestanding/font5x7.sura"],
    ...
    window_manager_foundation: {
      ...
      os_integration: "Six kernel-rendered application windows use the manager for focus, z-order, title-bar drag, close/reopen, bounds, Start/taskbar activation, active-window keyboard routing, and FAT32-backed desktop-state restore",
    ...
    desktop_applications: {
      ...
      applications: {
        ...
        file_explorer: ["bounded entry count and selection", "fixed CPL-3 selection worker with a bounded mailbox", "executed OS traverses FAT32 root and DOCS short-name directories and persists the selected row after returning to ring 0"],
        text_editor: ["caller-owned buffer", "printable text", "Enter", "Backspace", "dirty state", "fixed CPL-3 editing worker with a bounded mailbox", "executed OS loads and autosaves fixed-size NOTES.TXT"],
        calculator: ["unsigned integer input", "+", "-", "*", "/", "=", "C", "Backspace", "overflow, underflow, and division-by-zero error state", "fixed CPL-3 state worker with a bounded mailbox"],
      },
      example: "examples/os/desktop_apps_features.sura",
      os_integration: "registered windows 2..5, desktop/Start/taskbar launch and reopen, and active-app input routing; System Information, Terminal, File Explorer, Text Editor, and Calculator rendering remains in ring 0 while their state-transition functions run at CPL 3 through bounded mailboxes",
      executed_verification: "QEMU validates a System Information snapshot through its CPL-3 path before ring-0 rendering, edits and recognizes Terminal commands through the Terminal CPL-3 path before ring-0 rendering and privileged execution, mounts the FAT32 data disk, selects and opens DOCS through the File Explorer CPL-3 path before ring-0 FAT32 traversal, appends SURA NOTES through the Text Editor CPL-3 path and flushes it through AHCI after returning to ring 0, evaluates 50 - 31 = 19 through the Calculator CPL-3 path, verifies five distinct worker CR3 roots, requires app/input/result/storage and Ring-3 markers, and captures build/os/SuraOS-apps.ppm",
      limitations: ["all window rendering remains kernel-owned", "System Information, Terminal, File Explorer, Text Editor, and Calculator each have a separate ProcessAddressSpace and CR3 but use synchronous entry/return rather than UserProcessScheduler or timer preemption", "hardware snapshot gathering, terminal rendering and privileged commands, FAT32 traversal, and Text Editor persistence remain ring-0 operations after their workers return", "File Explorer supports bounded short-name directory traversal only", "Text Editor overwrites one fixed 512-byte file", "Calculator is unsigned integer only"],
    },
    cmos_rtc: {
    ...
    ps2_desktop_input: {
      ...
      keyboard: ["left and right Shift", "Caps Lock", "printable ASCII", "Enter", "Backspace", "extended-key tracking"],
      ...
      executed_verification: "tools/sura_os_screenshot.ps1 exercises Shift, Backspace, status, pointer movement, focus, title-bar drag, close, taskbar reopen, and Start-menu activation through QEMU QMP",
      limitations: ["polling rather than IRQ1/IRQ12 queues", "no key repeat policy, layout selection, or Unicode input", "no wheel or five-button packets", "no USB HID"],
    },
    context_primitives: {
      frame_bytes: 72,
    ...
    user_process_lifecycle: {
      ...
      switching: ["per-process CR3", "per-process TSS RSP0", "per-process user GS base", "timer preemption", "DPL-3 vector-130 voluntary yield"],
      lifecycle: ["checked creation from ProcessAddressSpace", "non-returning exit", "ring-3 page-fault termination with CR2/error capture", "state/result/fault lookup", "address-space destruction on reap"],
      library: "stdlib/freestanding/user_process.sura",
      example: "examples/os/user_process_features.sura",
      limitations: ["standard GDT selectors 8/27/35", "shared supervisor-only higher-half kernel code/data/TSS/stacks required", "no preemption while a runnable process executes trusted ring-0 code", "no fast-syscall blocking/resume conversion", "no FPU/SIMD or debug-state save", "no signals, priorities, SMP queues, PCID, KPTI, remote TLB shootdown, or NMI-safe entry", "scheduler remains compile-verified only; the desktop System Information, Terminal, File Explorer, Text Editor, and Calculator switch distinct ProcessAddressSpace CR3 roots synchronously without running UserProcessScheduler"],
    },
    ...
    nvme_library: {
      ...
      dma_contract: ["caller-owned contiguous page-aligned queues", "registered page-aligned data window", "PRP1 plus one direct PRP2 page", "4-KiB controller minimum page", "queue depth at most 64"],
      library: "stdlib/freestanding/nvme.sura",
      example: "examples/os/nvme_features.sura",
      limitations: ["no executed NVMe command proof", "no MSI/MSI-X or multiple I/O queue pairs", "no concurrent commands", "no PRP lists or SGLs", "no namespace-list selection", "no controller shutdown or abort/reset recovery", "no asynchronous events, IOMMU mapping, or zoned namespaces", "rejects separate LBA metadata"],
    },
    fat32_library: {
      ...
      executed_verification: "The OS mounts a generated FAT32 data image, lists root short names, enters DOCS and lists README.TXT through a bounded directory stack, returns through the parent entry, reads NOTES.TXT, overwrites and flushes NOTES/SETTINGS/DESKTOP fixed-size files, and remounts the preserved modified image",
      limitations: ["no cluster allocation/free", "no create, resize, delete, or rename", "no long file names", "no general string path parser; the OS supplies bounded click navigation", "no FAT repair or FSInfo update", "no journaling or locking"],
    },
    partition_libraries: {
    ...
    virtual_filesystem_library: {
      ...
      operations: ["mount", "unmount", "longest-prefix resolution", "open", "read", "write", "seek", "flush", "close", "sync"],
      library: "stdlib/freestanding/vfs.sura",
      example: "examples/os/vfs_features.sura",
      limitations: ["no allocation", "no internal synchronization", "no permissions or process ownership", "no path normalization, current directory, or symbolic links", "compile and image verification only"],
    },
    virtio_network_and_browser: {
      libraries: ["stdlib/freestanding/virtio_net.sura", "stdlib/freestanding/net.sura"],
      integration: ["os/network.sura", "os/sura_os.sura"],
      device: ["legacy-compatible VirtIO-net PCI discovery", "I/O BAR", "feature negotiation", "two 256-entry split virtqueues", "eight posted 2048-byte RX buffers", "single polling TX descriptor"],
      protocols: ["Ethernet II", "DHCP Discover/Offer/Request/ACK", "leased IPv4 address, subnet, gateway, and DNS configuration", "ARP request/reply", "IPv4 header build and checksum validation", "UDP", "DNS A query and compressed-name response traversal", "TCP SYN/SYN-ACK/ACK", "TCP payload acknowledgement and FIN", "HTTP/1.0 GET"],
      browser: ["sixth managed TEXT BROWSER window", "keyboard-editable hostname or http:// URL with path", "Enter-triggered DNS/TCP/HTTP navigation", "HTTP header removal", "bounded h1/p/br/a HTML tokenization", "separate heading, body, and link layout runs", "CSS body/div/h1/a selector colors", "CSS background, background-color, and color with #RGB or #RRGGBB", "desktop icon and taskbar activation", "QMP framebuffer capture"],
      executed_verification: ["SURA_OS_NETWORK_READY", "SURA_OS_DHCP_OK", "SURA_OS_ARP_OK", "SURA_OS_UDP_OK", "SURA_OS_DNS_OK", "SURA_OS_TCP_OK", "SURA_OS_HTTP_OK", "SURA_OS_BROWSER_APP_OK", "SURA_OS_BROWSER_CSS_OK", "SURA_OS_BROWSER_FOCUS_OK", "SURA_OS_BROWSER_URL_OK", "build/os/SuraOS-browser.ppm"],
      limitations: ["QEMU user networking is the executed environment", "polling only", "no IPv6", "bounded connections without retransmission, congestion control, or out-of-order reassembly", "no TLS/HTTPS or non-80 URL ports", "no nested DOM box layout, images, forms, or general CSS cascade/box/flex/grid", "no JavaScript, cookies, cache, history, or downloads"],
    },
    serial_and_vm_gate: {
    ...
    minimal_os_integration: {
      ...
      ui_assets: ["os/icons.sura", "seven embedded 16x16 palette-index raster images", "desktop, window-title, Start-menu, taskbar, and File Explorer rendering"],
      gate: "tools/sura_os_vm.ps1",
      output: ["build/os/SuraOS.efi", "build/os/SuraOS.img", "build/os/SuraData.img"],
      executed_path: ["UEFI entry", "GOP framebuffer discovery", "64 MiB-bounded backbuffer allocation", "AHCI and VirtIO-net DMA allocation", "COM1 initialization", "memory-map acquisition", "ExitBootServices", "pixel/shape/icon/bitmap-text desktop rendering", "double-buffer presentation", "cursor-only damage repair", "six managed application windows", "focus and z-order", "title-bar drag", "close-button hit dispatch", "desktop-bound clamping", "taskbar reopen", "expanded Start menu", "ICH9 AHCI discovery and ATA IDENTIFY", "MBR type-0xEF partition lookup", "FAT32 mount", "root and DOCS short-name directory traversal", "NOTES.TXT load and fixed-size autosave", "SETTINGS.CFG selected-row persistence", "DESKTOP.CFG six-window position/visibility/z-order/active-window persistence", "second-boot state restore", "VirtIO-net split queues", "DHCP Discover/Offer/Request/ACK", "leased IPv4/subnet/gateway/DNS configuration", "Ethernet ARP gateway and DNS discovery", "IPv4 UDP DNS A query", "TCP port-80 handshake", "HTTP/1.0 response receive", "keyboard-editable browser URL and second live navigation", "bounded h1/p/br/a HTML layout", "CSS body/div/h1/a colors with background/background-color/color and #RGB/#RRGGBB", "TEXT BROWSER framebuffer rendering", "reusable user-worker address-space setup", "Text Editor process-owned W^X code, writable/NX mailbox, guarded stack, and distinct ProcessAddressSpace CR3", "Text Editor CPL-3 SURA NOTES state transition with ring-0 FAT32 persistence after return", "SURA_OS_EDITOR_RING3_READY", "SURA_OS_EDITOR_RING3_OK", "SURA_OS_EDITOR_CR3_OK", "Calculator process-owned W^X code, writable/NX mailbox, guarded stack, and distinct ProcessAddressSpace CR3", "Calculator CPL-3 state transition for 50 - 31 = 19", "SURA_OS_CALCULATOR_RING3_READY", "SURA_OS_CALCULATOR_RING3_OK", "SURA_OS_CALCULATOR_CR3_OK", "CMOS RTC HH:MM taskbar clock", "active-window keyboard routing", "SURA_OS_FILES/EDITOR/CALCULATOR_APP_OK", "SURA_OS_EDITOR_INPUT_OK", "SURA_OS_CALCULATOR_RESULT_OK", "SURA_OS_STORAGE_READY", "SURA_OS_STORAGE_READ_OK", "SURA_OS_STORAGE_WRITE_OK", "SURA_OS_SETTINGS_READY", "SURA_OS_DESKTOP_STATE_READY", "SURA_OS_DIRECTORY_OK", "SURA_OS_NETWORK_READY", "SURA_OS_DHCP_OK", "SURA_OS_ARP_OK", "SURA_OS_UDP_OK", "SURA_OS_DNS_OK", "SURA_OS_TCP_OK", "SURA_OS_HTTP_OK", "SURA_OS_BROWSER_APP_OK", "SURA_OS_BROWSER_CSS_OK", "SURA_OS_BROWSER_FOCUS_OK", "SURA_OS_BROWSER_URL_OK", "SURA_OS_WINDOW_READY", "SURA_OS_WINDOW_FOCUS_OK", "SURA_OS_WINDOW_DRAG_OK", "SURA_OS_WINDOW_CLOSE_OK", "SURA_OS_WINDOW_REOPEN_OK", "SURA_OS_START_MENU_OK", "SURA_OS_RTC_OK", "SURA_OS_DESKTOP_OK", "physical page allocate/write/read/free self-check", "SURA_OS_KERNEL_READY", "polling PS/2 controller initialization", "46x14 graphical terminal command history at 2x font scale", "wrap, scroll, clear, and numeric output", "software mouse pointer movement", "COM1 command shell", "status and mem command validation", "shutdown command", "QEMU isa-debug-exit"],
      desktop_ring3_workers: {
        ...
        file_explorer: ["process-owned W^X code", "writable/NX mailbox", "guarded stack", "distinct ProcessAddressSpace CR3", "CPL-3 bounded selection validation", "ring-0 FAT32 traversal after return", "SURA_OS_FILES_RING3_READY", "SURA_OS_FILES_RING3_OK", "SURA_OS_FILES_CR3_OK"],
        text_editor: ["process-owned W^X code", "writable/NX mailbox", "guarded stack", "distinct ProcessAddressSpace CR3", "CPL-3 SURA NOTES state transition", "ring-0 FAT32 persistence after return", "SURA_OS_EDITOR_RING3_READY", "SURA_OS_EDITOR_RING3_OK", "SURA_OS_EDITOR_CR3_OK"],
        calculator: ["process-owned W^X code", "writable/NX mailbox", "guarded stack", "distinct ProcessAddressSpace CR3", "CPL-3 state transition for 50 - 31 = 19", "SURA_OS_CALCULATOR_RING3_READY", "SURA_OS_CALCULATOR_RING3_OK", "SURA_OS_CALCULATOR_CR3_OK"],
      },
      ...
      screenshots: ["build/os/SuraOS-windows.ppm after drag", "build/os/SuraOS-start-menu.ppm with reopened System Information", "build/os/SuraOS-apps.ppm with File Explorer, Text Editor, and Calculator", "build/os/SuraOS-browser.ppm with the fetched example.com body", "build/os/SuraOS-desktop.ppm after Terminal activation"],
      ...
      limitations: ["graphical desktop milestone, not a complete desktop OS", "polling PS/2, AHCI, VirtIO-net, and COM1 rather than interrupt-driven queues", "no resize, minimize, maximize, or scheduled Ring 3 application process windows", "System Information, Terminal, File Explorer, Text Editor, and Calculator have distinct CR3 roots but still return synchronously and are not managed by UserProcessScheduler", "Browser logic, window rendering, hardware snapshot gathering, privileged terminal actions, FAT32 traversal, and storage persistence remain kernel-owned", "fixed kernel-owned Start/taskbar entries", "FAT32 mutation is limited to fixed-size existing-file overwrite", "DHCP and HTTP are implemented but IPv6, TLS, TCP retransmission, congestion control, and out-of-order reassembly are not", "browser HTML/CSS support is a bounded semantic and color subset without nested DOM box layout, images, forms, general cascade, box model, flex, grid, or JavaScript", "does not execute every compile-verified freestanding subsystem", "no USB or audio"],
    },
    acpi_madt: {
    ...
    verification: ["tests/os_target_unit.cpp", "tests/freestanding_import_unit.cpp", "tools/sura_uefi_target_smoke.ps1", "tools/sura_ap_startup_smoke.ps1", "tools/sura_qemu_boot_gate.ps1", "tools/sura_ring3_qemu_gate.ps1", "tools/sura_os_vm.ps1", "tools/sura_os_screenshot.ps1", "tools/sura_os_data_disk.ps1", "os/sura_os.sura", "os/user_worker.sura", "os/user_system_info.sura", "os/user_terminal.sura", "os/user_file_explorer.sura", "os/user_text_editor.sura", "os/user_calculator.sura", "os/storage.sura", "os/network.sura", "stdlib/freestanding/process_memory.sura", "stdlib/freestanding/virtio_net.sura", "stdlib/freestanding/net.sura", "stdlib/freestanding/framebuffer.sura", "stdlib/freestanding/font5x7.sura", "stdlib/freestanding/text_terminal.sura", "stdlib/freestanding/window_manager.sura", "stdlib/freestanding/desktop_shell.sura", "stdlib/freestanding/desktop_apps.sura", "stdlib/freestanding/rtc.sura", "stdlib/freestanding/ps2.sura", "examples/os/framebuffer_features.sura", "examples/os/text_terminal_features.sura", "examples/os/window_manager_features.sura", "examples/os/desktop_shell_features.sura", "examples/os/desktop_apps_features.sura", "examples/os/rtc_features.sura", "examples/os/ps2_features.sura", "examples/os/freestanding_features.sura", "examples/os/memory_kernel.sura", "examples/os/process_elf_features.sura", "examples/os/user_process_features.sura", "examples/os/ring3_qemu_gate.sura", "examples/os/scheduler_features.sura", "examples/os/preemptive_timer_features.sura", "examples/os/syscall_features.sura", "examples/os/user_mode_features.sura", "examples/os/pci_features.sura", "examples/os/pcie_features.sura", "examples/os/block_features.sura", "examples/os/ahci_features.sura", "examples/os/nvme_features.sura", "examples/os/gpt_features.sura", "examples/os/partition_features.sura", "examples/os/fat32_features.sura", "examples/os/vfs_features.sura", "examples/os/qemu_boot_gate.sura", "examples/os/acpi_features.sura", "examples/os/ioapic_features.sura", "examples/os/ap_startup_features.sura"],
...
sections.push(section("freestanding", "OS 개발용 freestanding 기능",
  ...
  table(["영역", "현재 구현"], [
    ...
    ["사용자 프로세스", "fixed-capacity single-CPU round-robin, per-process CR3/TSS RSP0/user GS, timer/yield, exit, page-fault termination, reap"],
    ...
    ["그래픽 OS 통합", code("os/sura_os.sura") + "와 " + code("tools/sura_os_vm.ps1") + "; QEMU TCG에서 double-buffer desktop·AHCI/FAT32 영속 저장·VirtIO TCP/IP/HTTP·memory self-check와 COM1 shell 검증"],
    ...
    ["그래픽 터미널", code("text_terminal.sura") + "의 fixed ASCII cells, wrap, scroll, clear, number output와 framebuffer draw; 별도 CR3의 CPL 3 줄 편집·명령 인식과 ring 0 특권 명령 실행 검증"],
    ...
    ["VirtIO 네트워크·브라우저", code("virtio_net.sura") + "·" + code("net.sura") + "·" + code("os/network.sura") + "로 split queue, DHCP, ARP, IPv4, UDP DNS, TCP, HTTP/1.0, 주소 입력, h1/p/a HTML 레이아웃과 CSS 색상 하위 집합을 실행 검증"],
  ...
  paragraph("Sura freestanding 기반에는 메모리·페이지 테이블·프로세스·스케줄러·syscall·PCI/PCIe·ACPI·블록 장치·AHCI·NVMe·GPT/MBR·FAT32·VFS·VirtIO-net·Ethernet/DHCP/ARP/IPv4/UDP/DNS/TCP/HTTP·framebuffer·bitmap font·text terminal·window manager·desktop shell·File Explorer·Text Editor·Calculator·Text Browser·CMOS RTC·polling PS/2·serial·QEMU boot gate가 구현되어 있습니다. 현재 QEMU 실행 이미지는 여섯 개의 kernel-rendered 창, FAT32 하위 디렉터리와 자동 저장·재부팅 복원, DHCP 주소 임대, DNS와 HTTP 응답, 키보드로 입력한 HTTP 주소의 두 번째 탐색, h1/p/a HTML 레이아웃과 CSS 색상 하위 집합, 각각 별도 ProcessAddressSpace CR3와 bounded mailbox를 사용하는 System Information·Terminal·File Explorer·Text Editor·Calculator CPL 3 상태 전이, Start·taskbar activation, HH:MM clock과 active-window routing까지 실행 검증합니다. 다섯 작업자는 독립 CR3를 사용하지만 현재 요청마다 동기식으로 진입·복귀하며 UserProcessScheduler나 timer preemption으로 관리되지는 않습니다. 하드웨어 스냅샷 수집, 터미널 렌더링·특권 명령, FAT32 탐색·저장과 모든 창 렌더링은 ring 0에 남아 있습니다. 아직 없는 핵심 기능은 interrupt-driven input/network queue, resize/minimize/maximize와 scheduled Ring 3 application process를 포함한 완전한 interactive window system, FAT32 파일 생성·삭제·크기 변경·긴 이름, IPv6·완전한 TCP 재전송/혼잡 제어·TLS, 중첩 DOM box layout·이미지·폼·일반 CSS cascade/box/flex/grid·JavaScript 브라우저, USB HID, audio, accelerated GPU driver, SMP load balancing, complete per-AP lifecycle, FPU/SIMD process state, ARM64 freestanding backend입니다. 자세한 범위와 각 기능의 compile-only 또는 executed 검증 구분은 " + code("Guide/OS_DEVELOPMENT.md") + "와 이 문서의 machine-readable data에 있습니다.")
));
