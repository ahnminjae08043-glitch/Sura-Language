...
const machineFacts = {
  ...
  freestanding: {
    ...
    block_device_library: {
      ...
      limitations: ["caller owns storage, contexts, buffers, and serialization", "bool public result does not preserve detailed callback status", "first matching UEFI protocol only", "UEFI adapter invalid after ExitBootServices", "no native AHCI/NVMe driver", "compile and image verification only"],
    },
    fat32_library: {
    ...
    virtual_filesystem_library: {
      ...
      paths: "caller-owned UTF-8 byte buffers with explicit lengths",
