...
sections.push(section("freestanding", "OS 개발용 freestanding 기능",
  ...
  paragraph("GS-relative per-CPU 저장소와 runtime xAPIC/x2APIC 접근, EOI, IPI, INIT, SIPI 전송, 16-bit real-mode에서 64-bit Sura entry로 이어지는 AP trampoline과 bounded startup helper, 4-level page-table entry와 CR3/TLB primitive, UEFI memory-map bitmap physical allocator, conflict-checked virtual-memory walker, 72-byte cooperative context frame, single-CPU cooperative scheduler, checked ring-0 interrupt-frame scheduler와 APIC/PIT/TSC-deadline timer helper, software-interrupt syscall dispatcher, checked ring-3 IRETQ entry와 SYSCALL/SYSRETQ helper, legacy PCI configuration library, checked block-device ABI와 RAM disk 및 boot-stage UEFI Block I/O adapter, polling AHCI SATA와 NVMe DMA foundations, CRC-checked primary/backup GPT와 primary-MBR discovery, 제한된 FAT32 reader/same-size overwrite writer, fixed-capacity VFS dispatch, checked ACPI MADT discovery, 16550 serial diagnostics, GPT/FAT32 UEFI boot-disk builder와 QEMU/OVMF gate가 구현되어 있습니다. 아직 없는 기능은 executed AP-startup coverage와 complete per-AP initialization lifecycle, automatic per-CPU TSS/IST 관리, FPU/SIMD context-switch 정책, automatic intermediate page-table allocation/reclamation, per-process address-space와 executable loader, safe user copy-in/out와 process fault/exit policy, SMP run queue와 load balancing, user-mode preemption, executed timer/context-switch verification, KPTI와 speculative-entry hardening, PCIe ECAM/resource allocation, network/USB/graphics/audio와 다른 device-specific drivers, extended MBR/GPT repair/partition creation-resizing, filesystem allocation/create/resize/delete/long-name/recovery/locking, ARM64 freestanding backend입니다. " + code("examples/os/freestanding_features.sura") + ", " + code("examples/os/memory_kernel.sura") + ", " + code("examples/os/scheduler_features.sura") + ", " + code("examples/os/preemptive_timer_features.sura") + ", " + code("examples/os/syscall_features.sura") + ", " + code("examples/os/user_mode_features.sura") + ", " + code("examples/os/pci_features.sura") + ", " + code("examples/os/block_features.sura") + ", " + code("examples/os/ahci_features.sura") + ", " + code("examples/os/nvme_features.sura") + ", " + code("examples/os/gpt_features.sura") + ", " + code("examples/os/partition_features.sura") + ", " + code("examples/os/fat32_features.sura") + ", " + code("examples/os/vfs_features.sura") + ", " + code("examples/os/acpi_features.sura") + ", " + code("examples/os/ap_startup_features.sura") + ", " + code("examples/os/qemu_boot_gate.sura") + "는 기능 시험이며 완성 OS가 아닙니다. 자세한 계약은 " + code("Guide/OS_DEVELOPMENT.md") + "에 있습니다.")
));
