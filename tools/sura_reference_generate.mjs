...
const machineFacts = {
  ...
  freestanding: {
    ...
    desktop_applications: {
      ...
      os_integration: "registered windows 3..5, desktop/Start/taskbar launch and reopen, and active-app input routing; File Explorer, Text Editor, and Calculator rendering remains in ring 0 while their state-transition functions run at CPL 3 through bounded mailboxes",
      executed_verification: "QEMU mounts the FAT32 data disk, selects and opens DOCS through the File Explorer CPL-3 path before ring-0 FAT32 traversal, appends SURA NOTES through the Text Editor CPL-3 path and flushes it through AHCI after returning to ring 0, evaluates 50 - 31 = 19 through the Calculator CPL-3 path, verifies three distinct worker CR3 roots, requires app/input/result/storage and Ring-3 markers, and captures build/os/SuraOS-apps.ppm",
      limitations: ["all window rendering remains kernel-owned", "File Explorer, Text Editor, and Calculator each have a separate ProcessAddressSpace and CR3 but use synchronous entry/return rather than UserProcessScheduler or timer preemption", "FAT32 traversal and Text Editor persistence remain ring-0 operations after their workers return", "File Explorer supports bounded short-name directory traversal only", "Text Editor overwrites one fixed 512-byte file", "Calculator is unsigned integer only"],
    },
    cmos_rtc: {
    ...
    fast_syscalls_and_user_mode: {
      ...
      desktop_process_gate: ["File Explorer, Text Editor, and Calculator use distinct ProcessAddressSpace CR3 roots", "shared kernel PML4 entries have U/S cleared", "process-owned W^X code", "writable/NX mailboxes", "guarded writable/NX user stacks", "SURA_OS_FILES_CR3_OK, SURA_OS_EDITOR_CR3_OK, and SURA_OS_CALCULATOR_CR3_OK after the interrupt handler observes each process root and restores the kernel root"],
      limitations: ["entry primitive does not automatically select an address space or validate syscall pointers", "standalone executed gate is one fixed test context, not a scheduled desktop application process", "desktop File Explorer, Text Editor, and Calculator execute CR3 switching but not ELF loading, timer preemption, or UserProcessScheduler", "generic interrupt wrapper is not an NMI/MCE paranoid-entry implementation", "no KPTI or comprehensive speculative-entry hardening", "no FPU/SIMD save"],
    },
...
sections.push(section("freestanding", "OS 개발용 freestanding 기능",
  ...
  table(["영역", "현재 구현"], [
    ...
    ["기본 데스크톱 앱", code("desktop_apps.sura") + "의 File Explorer·Text Editor·Calculator state; QEMU에서 각각 별도 CR3를 사용하는 CPL 3 폴더 선택, editor 입력·저장, 계산 경로 50-31=19 실행 검증"],
  ...
  paragraph("Sura freestanding 기반에는 메모리·페이지 테이블·프로세스·스케줄러·syscall·PCI/PCIe·ACPI·블록 장치·AHCI·NVMe·GPT/MBR·FAT32·VFS·VirtIO-net·Ethernet/DHCP/ARP/IPv4/UDP/DNS/TCP/HTTP·framebuffer·bitmap font·text terminal·window manager·desktop shell·File Explorer·Text Editor·Calculator·Text Browser·CMOS RTC·polling PS/2·serial·QEMU boot gate가 구현되어 있습니다. 현재 QEMU 실행 이미지는 여섯 개의 kernel-rendered 창, FAT32 하위 디렉터리와 자동 저장·재부팅 복원, DHCP 주소 임대, DNS와 HTTP 응답, 키보드로 입력한 HTTP 주소의 두 번째 탐색, h1/p/a HTML 레이아웃과 CSS 색상 하위 집합, 각각 별도 ProcessAddressSpace CR3와 bounded mailbox를 사용하는 File Explorer·Text Editor·Calculator CPL 3 상태 전이, Start·taskbar activation, HH:MM clock과 active-window routing까지 실행 검증합니다. 세 작업자는 독립 CR3를 사용하지만 현재 요청마다 동기식으로 진입·복귀하며 UserProcessScheduler나 timer preemption으로 관리되지는 않습니다. FAT32 탐색·저장과 모든 창 렌더링은 ring 0에 남아 있습니다. 아직 없는 핵심 기능은 interrupt-driven input/network queue, resize/minimize/maximize와 scheduled Ring 3 application process를 포함한 완전한 interactive window system, FAT32 파일 생성·삭제·크기 변경·긴 이름, IPv6·완전한 TCP 재전송/혼잡 제어·TLS, 중첩 DOM box layout·이미지·폼·일반 CSS cascade/box/flex/grid·JavaScript 브라우저, USB HID, audio, accelerated GPU driver, SMP load balancing, complete per-AP lifecycle, FPU/SIMD process state, ARM64 freestanding backend입니다. 자세한 범위와 각 기능의 compile-only 또는 executed 검증 구분은 " + code("Guide/OS_DEVELOPMENT.md") + "와 이 문서의 machine-readable data에 있습니다.")
));
