...
const machineFacts = {
  ...
  execution: {
    ...
    garbage_collector: "mark-sweep; process-global heap; per-type free-list pools for strings, arrays, dictionaries and instances; allocation takes the runtime mutex only once an embedded context exists",
...
sections.push(section("overview", "문서 개요",
  ...
  "</div>" +
  paragraph("Sura source 확장자는 " + code(".sura") + "입니다. 값은 NaN-boxed dynamic Value로 표현하고, register VM과 mark-sweep GC(문자열·배열·딕셔너리·인스턴스는 타입별 free list 풀에서 할당)가 실행하며, 딕셔너리는 삽입 순서를 지키는 compact open-addressing 표(항목은 4부터 두 배씩 커지는 청크에, 인덱스는 2의 거듭제곱 크기의 선형 탐사 표)입니다. 순회와 출력은 삽입 순서를 따르고, 삭제된 항목은 인덱스 재구성 때 압축되며, 문자열 키의 해시는 문자열 객체에 한 번만 계산해 캐시합니다. 할당은 임베딩 API로 컨텍스트가 만들어지기 전까지 런타임 뮤텍스를 잡지 않습니다(명령줄 엔진은 단일 스레드). GC heap은 프로세스 전역입니다.") +
...
sections.push(section("performance", "성능과 검증 기록",
  ...
  paragraph("native emitter는 Win64 x64 전체 backend, Linux x86-64 System V baseline, little-endian Windows/Linux/macOS ARM64 AAPCS64 baseline을 구현합니다. 두 baseline(x86-64는 8차, ARM64는 5차 개정)은 상수·이동·정적으로 숫자임이 증명된 덧셈·뺄셈·곱셈·단항 음수·여섯 가지 비교·0이 아님이 증명된 나눗셈·가드된 전역 읽기·순수 숫자 함수 사이의 네이티브 직접 호출(재귀 포함)을 인라인으로 내보내고, 타입이 증명되지 않은 산술·비교·분기는 실행 시 NaN-box 태그를 검사해 두 피연산자가 숫자면 인라인으로 계산합니다. 배열 인덱싱(범위 검사 포함)·push·len·딕셔너리 has도 인라인이고, x86-64 baseline은 인스턴스 필드 읽기·쓰기도 사이트의 인라인 캐시(클래스·필드 번호)를 실행 시점에 읽어 인라인으로 처리하며, 문자열·딕셔너리 읽기와 쓰기·in·for ... in·일반 호출·그 밖의 메서드 호출은 17개 VM helper를 네이티브 코드에서 호출합니다. x86-64 두 tier(Win64 전체 컴파일러와 System V baseline)는 루프 레지스터 캐시를 공유합니다: 바깥에서 뛰어드는 분기가 없는 루프 안에서 산술이 읽고 쓰는 레지스터는 XMM 레지스터에도 상주하고(Win64는 callee-saved XMM6–XMM15를 프롤로그에서 저장하고 UNWIND_INFO에 기록, System V는 호출 뒤 재적재), baseline은 모든 정의가 메모리 슬롯도 함께 쓰므로(store-through) helper·GC·deopt·예외가 항상 최신 메모리를 보고, Win64 전체 컴파일러는 캐시가 직접 내보내는 연산(상수·이동·산술·비교·나눗셈·나머지·배열 인덱싱·인라인 push/len/has·가상 레코드 접근·분기)만으로 된 루프에서는 XMM에만 두고(write-back) helper 호출 직전과 루프를 벗어나는 경로(조건 탈출은 exit stub)에서만 되씁니다. 전체 컴파일러는 결과가 바로 뒤 조건분기에만 쓰이는 비교를 플래그 분기로 융합하고, 나눗셈(제수 0·NaN은 helper)과 정수값 나머지(idiv, 그 외 fmod helper)를 인라인으로 내보내며, 클로저의 숫자로 쓰이는 매개변수는 진입에서 한 번 검사해 숫자가 아니면 deopt sentinel로 인터프리터에 넘깁니다(인터프리터가 이미 숫자 아님을 본 매개변수는 가드하지 않음). 두 x86-64 tier 모두 순서 비교를 바로 뒤 조건분기와 융합하고, baseline의 자기 재귀 직접 호출은 콜리가 쓰기 전에 읽을 수 있는 프레임 슬롯만 0으로 채웁니다. 전체 컴파일러는 INDEX_GET/SET의 키·hoist된 컨테이너로만 쓰이는 MOVE 임시를 접어 원본 레지스터를 읽고, 함수 안 전역 읽기는 범위·nil 검사 뒤 인라인입니다. 딕셔너리는 마지막으로 조회한 문자열 키 객체와 그 슬롯을 기억해(삭제·재구성 시 무효) 같은 키의 has·읽기·쓰기가 한 번만 탐사합니다. 숫자 태그 검사는 두 tier 모두 unordered 자기 비교(ucomisd x,x; jp) 두 명령으로 하며(NaN-box 태그는 모두 NaN 패턴, 진짜 NaN 숫자는 정확한 slow path), 순수 baseline 본문은 가드된 전역을 진입에서 한 번만 검사하고 본문 안에서는 단순 로드합니다. 메서드 호출이 없어 배열 크기가 변할 수 없는 write-back 루프에서는 끌어올린 컨테이너의 data 포인터를 GPR에, 끝 포인터를 프레임 슬롯에 두어 원소 접근이 lea 하나와 비교 하나로 줄어듭니다. 산술(+ - * /)은 먼저 계산하고 결과가 NaN인지 한 번만 검사합니다: NaN 박싱된 비숫자는 모두 NaN 패턴이라 연산을 통과하면 결과도 NaN이 되므로, 결과가 NaN이 아니면 두 피연산자가 모두 숫자였음이 증명됩니다. 진짜 NaN 결과(NaN 피연산자, 무한대끼리 빼기, 0 곱하기 무한대)도 문자열이나 객체와 같은 느린 경로로 가서 인터프리터와 똑같은 값을 냅니다. 순서·동등 비교는 ucomisd가 내는 unordered 플래그만 보면 되므로 태그 검사가 없고, 나머지 연산은 정수 왕복 검사가 이미 NaN을 걸러냅니다. 끌어올린 컨테이너는 접근마다 널 검사를 하지 않습니다(프리헤더가 배열이 아니면 끝 포인터도 0으로 두고 빈 벡터는 끝이 데이터와 같아, 범위 비교가 모든 인덱스를 걸러냅니다). 바로 뒤 명령만 읽고 그 뒤로 죽는 숫자 상수는 프레임 슬롯에 쓰지 않고 소비하는 산술·비교가 상수 배열에서 직접 읽습니다(소비자가 helper로 빠질 수 있으면 피연산자를 프레임에서 읽으므로 접지 않습니다). 루프 안 모든 접근이 같은 키를 쓰고 그 키가 정수 step으로만, 그리고 모든 접근보다 뒤에서 한 번만 늘어나면, 끌어올린 레지스터가 배열 시작 대신 원소 포인터를 듭니다: 프리헤더가 data + trunc(키)*8로 채우고 반복마다 step*8을 더하므로 접근은 경계 비교와 이동뿐입니다. 키가 [0, 2^31)의 정수가 아니면 포인터를 끝에 두어 그 루프의 모든 접근이 helper로 가고, 음수 인덱스는 인터프리터처럼 뒤에서부터 셉니다. 치환된 생성자는 호출 대상 레지스터를 읽지 않으므로 그 값만 적재하던 전역 읽기(클래스 이름이면 매번 helper)는 내보내지 않고, 치환 계획이 값이 읽히지 않음을 보장하는 레코드 별칭 복사와 결과 레지스터 쓰기도 생략합니다. 바로 앞 명령으로 알려진 2의 거듭제곱 상수로 나누기는 정확한 역수 곱하기(비트 단위로 같은 결과, 0 검사 없음)로, 그 나머지는 음이 아닌 정수 피제수의 하위 비트로 계산하고 나머지 경우는 helper의 fmod가 맡습니다. 루프가 쓰지 않는 배열 컨테이너(컴파일러가 그것을 복사해 넣는 임시 레지스터 포함)의 ‘배열인가’ 검사는 pre-header에서 한 번만 하고 GCArray*(배열이 아니면 0)를 callee-saved GPR(R14·R15, 전체 컴파일러는 RSI·RDI까지)에 두며, 각 접근은 null 검사·인덱스 변환·경계 검사만 합니다. 원소 포인터와 길이는 매 접근마다 다시 읽으므로 루프 안 push도 안전하고, 헤더로 들어오는 바깥 분기는 pre-header로 우회합니다. Win64 전체 컴파일러의 숫자 증명은 배열·호출·객체 명령을 만나도 유지되며(그 명령이 쓰는 레지스터만 미지수로 둠) 숫자 동등 비교도 인라인입니다. 루프 안에서 plain 생성자로 만들어 필드 읽기·쓰기로만 쓰고 루프 밖으로 나가지 않는 레코드는 할당하지 않고 필드를 프레임 슬롯에 둡니다(배열에 담기거나 호출에 넘겨지거나 루프 뒤에 읽히면 그대로 할당). helper가 예외를 내면 그 명령에서 register VM으로 되돌아가 VM과 같은 오류를 냅니다. 네이티브 직접 호출은 callee가 helper에 닿을 수 있을 때만 프레임을 VM 값 스택에 올리고(하이브리드 프레임), fib처럼 helper에 닿지 않는 callee는 기계 스택을 씁니다. Windows x64는 호출이 있는 순수 숫자 함수(fib 같은 재귀)만 baseline으로 보내고, 호출 없는 루프 본문을 포함한 나머지는 인라인 캐시를 가진 전체 컴파일러가 맡습니다. array.sort는 모든 원소가 숫자면 IEEE 비트 패턴 기수 정렬(같은 바이트 패스 생략)이고, 모듈 함수 호출(string.len 등)은 호출 사이트가 (모듈 사전, 함수) 쌍을 캐시합니다. macOS x86-64에는 native backend가 없습니다. Vec2/Vec3 benchmark의 strict counted-loop shortcut은 인식된 top-level loop, closure identity, 정확한 field-copy constructor, Vec2/Vec3 layout, add/scale/cross와 step/step3 bytecode graph, numeric input, escape·non-alias 조건을 모두 증명한 경우에만 사용합니다. 하나라도 맞지 않으면 원래 VM/native bytecode path로 실행합니다. 한 번 이상 반복한 shortcut은 원래 position을 변경하지 않고 fresh result instance를 저장하므로 원래 객체를 가리키는 alias의 field가 바뀌지 않으며, 0회 반복은 원래 identity를 유지합니다. 이 경로는 일반 사용자 loop 최적화가 아닙니다. 일반 control-flow deoptimization, 범용 escape analysis, 루프 레지스터 캐시를 넘어서는 범용 register allocation, loop-invariant code motion(배열 컨테이너 검사 밖의 가드 끌어올리기 포함)은 구현되지 않았습니다. Windows x64 native helper frame은 UNWIND_INFO를 등록합니다.")
));
