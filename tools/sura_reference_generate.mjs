...
const machineFacts = {
  ...
  freestanding: {
    ...
    window_manager_foundation: {
      ...
      operations: ["visible and active state", "z-order", "focus", "top-window hit testing", "title-bar drag", "close-button hit testing", "desktop-bound clamping"],
      example: "examples/os/window_manager_features.sura",
      os_integration: "Terminal and System Information windows use the manager for focus, z-order, title-bar drag, close, bounds, and active-terminal keyboard routing",
      verification: "compile and EFI image verification plus QEMU PS/2 focus, drag, close, serial-marker, and overlapping-window screenshot verification",
      limitations: ["caller owns rendering and application contents", "no resize, minimize, maximize, reopen, taskbar buttons, or application processes"],
    },
    ps2_desktop_input: {
      ...
      executed_verification: "tools/sura_os_screenshot.ps1 exercises Shift, Backspace, status, pointer movement, focus, title-bar drag, and close through QEMU QMP, then requires keyboard, mouse, and managed-window markers",
    ...
    minimal_os_integration: {
      ...
      executed_path: ["UEFI entry", "GOP framebuffer discovery", "64 MiB-bounded backbuffer allocation", "COM1 initialization", "memory-map acquisition", "ExitBootServices", "pixel/shape/icon/bitmap-text desktop rendering", "double-buffer presentation", "Terminal and System Information managed windows", "focus and z-order", "title-bar drag", "close-button hit dispatch", "desktop-bound clamping", "active-terminal keyboard routing", "SURA_OS_WINDOW_READY", "SURA_OS_WINDOW_FOCUS_OK", "SURA_OS_WINDOW_DRAG_OK", "SURA_OS_WINDOW_CLOSE_OK", "SURA_OS_DESKTOP_OK", "physical page allocate/write/read/free self-check", "SURA_OS_KERNEL_READY", "polling PS/2 controller initialization", "46x14 graphical terminal command history at 2x font scale", "wrap, scroll, clear, and numeric output", "software mouse pointer movement", "COM1 command shell", "status and mem command validation", "shutdown command", "QEMU isa-debug-exit"],
      ...
      screenshots: ["build/os/SuraOS-windows.ppm before close", "build/os/SuraOS-desktop.ppm after close"],
      ...
      limitations: ["graphical desktop milestone, not a complete desktop OS", "polling PS/2 and COM1 input rather than interrupt-driven queues", "no resize, minimize, maximize, reopen, taskbar-button input, or application process windows", "does not execute every compile-verified freestanding subsystem", "no user program execution, persistent filesystem mutation, network, USB, audio, or browser"],
    },
    acpi_madt: {
...
sections.push(section("freestanding", "OS 개발용 freestanding 기능",
  ...
  table(["영역", "현재 구현"], [
    ...
    ["창 관리자", code("window_manager.sura") + "의 focus, z-order, hit test, title-bar drag, close, screen-bound clamp; 두 OS 창과 QEMU PS/2 입력으로 실행 검증"],
  ...
  paragraph("Sura freestanding 기반에는 메모리·페이지 테이블·프로세스·스케줄러·syscall·PCI/PCIe·ACPI·블록 장치·AHCI·NVMe·GPT/MBR·FAT32·VFS·framebuffer·bitmap font·text terminal·window manager·polling PS/2·serial·QEMU boot gate가 구현되어 있습니다. 현재 QEMU 실행 이미지는 그래픽 데스크톱, graphical terminal의 input/output/wrap/scroll/clear, software mouse pointer, 두 창의 focus/z-order/title-bar drag/close/bounds와 active-terminal keyboard routing까지 실행 검증합니다. 아직 없는 핵심 기능은 interrupt-driven input queue, resize/minimize/maximize/reopen/taskbar-button input과 application process를 포함한 완전한 interactive window system, user application execution, persistent filesystem create/delete, network/browser, USB HID, audio, accelerated GPU driver, SMP load balancing, complete per-AP lifecycle, FPU/SIMD process state, ARM64 freestanding backend입니다. 자세한 범위와 각 기능의 compile-only 또는 executed 검증 구분은 " + code("Guide/OS_DEVELOPMENT.md") + "와 이 문서의 machine-readable data에 있습니다.")
));
