...
$source = Join-Path $root "examples/os/window_server_features.sura"
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
    throw "Window Server QEMU source was not found: $source"
}
...
$arguments = @{
    ...
    ExpectedEfiText = "Sura window server test"
    ExpectedMarker = "SURA_WINDOW_SERVER_OK"
    ExpectedExitCode = 33
}
if ($CompileOnly) {
