...
$source = Join-Path $root "examples/os/process_syscall_features.sura"
...
if (-not (Test-Path -LiteralPath $source -PathType Leaf)) {
    throw "Process syscall QEMU source was not found: $source"
}
...
$arguments = @{
    ...
    ExpectedEfiText = "Sura checked process syscall boundary test"
    ExpectedMarker = "SURA_PROCESS_SYSCALL_OK"
    ExpectedExitCode = 33
}
if ($CompileOnly) {
