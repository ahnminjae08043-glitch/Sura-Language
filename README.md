...
새 기능의 범위와 안정 승격 조건은 [기능 범위](SCOPE.md), 빌드·테스트·패키지 기여 절차는 [기여 안내](CONTRIBUTING.md)에 있습니다. 채택 지표 기준의 분기별 계획은 [로드맵](ROADMAP.md)에 있습니다.
...
- `examples/showcase`: 핵심 기능을 보여주는 자기 검증 예제
- `tests`: 언어와 런타임 테스트
...
- 네이티브 JIT: Win64 x86-64 부분 컴파일, Linux x86-64 System V baseline, little-endian Windows/Linux/macOS ARM64 AAPCS64 baseline. 두 baseline은 상수·이동·숫자로 증명된 `+`, `-`, `*`, 단항 `-`, 비교 6종과 0이 아닌 제수로 증명된 `/`를 처리하고, x86-64(Windows·Linux)는 여기에 가드된 전역 읽기와 순수 숫자 함수 사이의 네이티브 직접 호출(재귀 포함)을 더하고, Windows는 baseline이 거부한 함수를 기존 전체 컴파일러로 넘긴다. 그 밖의 bytecode는 register VM으로 fallback
