...
struct CompilerEnv {
    ...
    uint16_t alloc_reg() {
        if (next_reg >= 65535) throw std::runtime_error("?®Ïàò/Î∏îÎ°ù ??Í∞Ä???àÏ??§ÌÑ∞ Ï¥àÍ≥º (ÏµúÎ? 65535Í∞?");
        uint16_t r = next_reg++;
...
class JitCompiler {
    ...
    // ?Ä?Ä Î£®ÌîÑ Ïª®ÌÖç?§Ìä∏ ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
    struct LoopCtx {
    ...
    // ?Ä?Ä Î≥Ä???ùÎ≥Ñ Î≥¥Ï°∞ ?®Ïàò ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
    ...
    // ?Ä?Ä Î≥¥Ï°∞ ?®Ïàò ?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä?Ä
...
public:
    void compile_expr(const Expr* e, uint16_t target) {
        ...
        case NK::ARRAY_LIT: {
            ...
            uint16_t start_reg = current_env->next_reg; // ?ÑÏû¨ ?†Îãπ???àÏ??§ÌÑ∞????(Î∞∞Ïó¥ ?êÏÜå ?ÑÏãú ?Ä??
            ...
            current_env->next_reg = start_reg; // ?ÑÏãú ?àÏ??§ÌÑ∞ ?¥Ï†ú
            break;
        }
        case NK::DICT_LIT: {
    ...
    void compile_stmt(const Stmt* s) {
        ...
        case NK::IF: {
            ...
            compile_stmt(is->then_block.get()); // SuraBlock Ï≤òÎ¶¨ ?¨Ìï®??
            if (is->else_block) {
                size_t end_jump = chunk.emit(JitOp::JUMP, 0, 0, 0, 0, -1, ln);
        ...
        case NK::CMD: {
            ...
            if (c->cmd == "print") chunk.emit(JitOp::PRINT, args_start, 0, 0, (int)c->args.size(), -1, ln);
                ...
                uint16_t dest = alloc_reg(); // ?¥Ïû• Î™ÖÎ†π?¥ÎèÑ Í≤∞Í≥ºÎ•?Î∞òÌôò?????àÏùå
                chunk.emit(JitOp::CALL_BUILTIN, dest, args_start, 0, (int)c->args.size(), chunk.add_string(c->cmd + std::string(1, '\0') + ident_info), ln);
                // Í≤∞Í≥º??dest???®Ïñ¥ÏßÄÍ≥?Î¨¥Ïãú?òÍ±∞???ÑÏöî?òÎ©¥ ?úÏö©??Î≥¥ÌÜµ CMD??Î¨¥Ïãú??
                current_env->next_reg -= 1;
            }
            current_env->next_reg = args_start; // ?∏Ïûê???ÑÏãú ?àÏ??§ÌÑ∞ Ï¶âÏãú Î∞òÌôò 
            break;
        }
        case NK::CLASS_DEF: {
