...
class TypeChecker {
    ...
    TypeAnnot                                        current_return; // ?�재 ?�수 반환 ?�??
    ...
    TypeAnnot infer(const Expr* e) {
        ...
        switch (e->kind) {
            ...
            case NK::BIN_OP: {
                ...
                // 비교 ?�산 ??불리??                if (b->op == "==" || b->op == "!=" || b->op == "<" || b->op == ">" ||
        ...
        switch (s->kind) {
            ...
            case NK::REPEAT: {
                ...
                if (cnt.present && cnt.kind != SType::NUMBER)
                    error(rs->line, "repeat 반복 ?�수???�자?�야 ?�니??);
            ...
            case NK::FOREACH: {
                ...
                env.push();
                env.set(fe->var, TypeAnnot{}); // ?�소 ?�??미�? ??ANY
            ...
            case NK::FUNC_DEF: {
                ...
                // param_types 가 부족하�?ANY �?채우�?                while (sig.param_types.size() < fd->params.size())
                    sig.param_types.push_back(TypeAnnot{});

                func_sigs[fd->name] = sig;

                // ?�수 바디 검??                TypeAnnot prev_ret = current_return;
