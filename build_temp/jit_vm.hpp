...
// ?��??�터 ?�레??struct CallFrame {
    std::vector<Value> registers;
    GCClosure* closure = nullptr; // ?�행 중인 ?�로?� (Upvalue ?�근??
    size_t ip = 0;
    size_t end_ip = 0;
    const JitChunk* chunk = nullptr;
};

// ?�외 구조�?struct JitReturn { Value value; };
struct JitThrow { std::string message; int line; };

// ?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═
// 가??머신 (Inline Caching & Tracing GC)
// ?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═?�═
class JitVM {
    std::vector<Value> globals;
    std::vector<GCUpvalue*> open_upvalues; // ?�히지 ?��? Upvalue 목록
    ...
    void run_gc() {
        ...
        // 4. Mark Root - ?�로?� (?�재 ?�로?�??GC 객체?��?�?�??�회?�서 ?�동 ?�캔??
        // 5. ?�래???�폴??        for (auto& [cname, c_info] : rt_classes) {
...
private:
    ...
    const JitMethodInfo* find_method(const std::string& cls, const std::string& method) {
        ...
        while (!cur.empty() && rt_classes.count(cur)) {
            ...
            if (method == "init" && c.methods.count("?�성??)) return &c.methods["?�성??];
            if (method == "?�성?? && c.methods.count("init")) return &c.methods["init"];
            cur = c.parent;
        }
        return nullptr;
    }
    ...
    Value execute_frame(CallFrame& frame) {
        ...
        while (frame.ip < frame.end_ip) {
            ...
            case JitOp::DEF_CLASS: {
                ...
                // inst.str_idx???�제 global index ?�며, ?�래???�름?� chunk.global_names???�음
                ...
                if (R[b].is_inst()) {
                    ...
                    if (inst.ic_cache != -1 && inst_obj->fields.size() > (size_t)inst.ic_cache) {
                        ...
                        if (offset != -1) {
                            ...
                            inst.ic_cache = offset; // ?�음 �?루프?�는 캐시 Hit ?�도�?명령???�구 변�?(IC!)
                        } else {
                            R[a] = Value::nil();
                        }
                    }
            ...
            case JitOp::NEW_INSTANCE: {
                ...
                auto ctor = find_method(cls, "?�성??); if (!ctor) ctor = find_method(cls, "init");
                if (ctor) {
                    CallFrame new_frame;
