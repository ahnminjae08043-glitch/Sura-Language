...
// ?àÏ??§ÌÑ∞ ?ÑÎ†à??struct CallFrame {
    std::vector<Value> registers;
    GCClosure* closure = nullptr; // ?§Ìñâ Ï§ëÏù∏ ?¥Î°ú?Ä (Upvalue ?ëÍ∑º??
    size_t ip = 0;
    size_t end_ip = 0;
    const JitChunk* chunk = nullptr;
};

// ?àÏô∏ Íµ¨Ï°∞Ï≤?struct JitReturn { Value value; };
...
// Í∞Ä??Î®∏Ïã† (Inline Caching & Tracing GC)
// ?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê?ê‚ïê
class JitVM {
    std::vector<Value> globals;
    std::vector<GCUpvalue*> open_upvalues; // ?´ÌûàÏßÄ ?äÏ? Upvalue Î™©Î°ù
    ...
    void run_gc() {
        ...
        // 3. Mark Root - ?úÏÑ± ?ÑÎ†à?ÑÏùò Ï≤?Å¨ Î≥¥Ìò∏(?ÅÏàò ?Ä, ?¥Îûò???®Ïàò ?îÌè¥??
        for (auto* frame : active_frames) {
            if (frame->chunk) {
        ...
        // 4. Mark Root - ?¥Î°ú?Ä (?ÑÏû¨ ?¥Î°ú?Ä??GC Í∞ùÏ≤¥?¥Î?Î°?Í∞??úÌöå?êÏÑú ?êÎèô ?§Ï∫î??
        // 5. ?¥Îûò???îÌè¥??        for (auto& [cname, c_info] : rt_classes) {
...
public:
    ...
    Value run(const JitChunk& main_chunk) {
        ...
        // globals Î∞∞Ïó¥ ?¨Í∏∞Î•?Ï≤?Å¨???ÑÏó≠ Î≥Ä??Î™©Î°ù ?¨Í∏∞??ÎßûÍ≤å Ï°∞Ï†ï
        ...
        try {
            ...
            throw; // Bubble up to main for visual report
...
private:
    ...
    void close_upvalues(Value* last_local) {
        for (auto it = open_upvalues.begin(); it != open_upvalues.end(); ) {
            ...
            if (uv->location >= last_local) {
                uv->closed = *(uv->location); // Í∞?Î≥µÏÇ¨ (?´Í∏∞)
                uv->location = nullptr;
    ...
    const JitMethodInfo* find_method(const std::string& cls, const std::string& method) {
        ...
        while (!cur.empty() && rt_classes.count(cur)) {
            ...
            if (method == "init" && c.methods.count("?ùÏÑ±??)) return &c.methods["?ùÏÑ±??];
            if (method == "?ùÏÑ±?? && c.methods.count("init")) return &c.methods["init"];
            cur = c.parent;
        }
        return nullptr;
    }
    ...
    Value execute_frame(CallFrame& frame) {
        ...
        while (frame.ip < frame.end_ip) {
            ...
            // Inline caching requires patching the instruction!
            ...
            case JitOp::DEF_CLASS: {
                ...
                // inst.str_idx???¥Ï†ú global index ?¥Î©∞, ?¥Îûò???¥Î¶Ñ?Ä chunk.global_names???àÏùå
                ...
                if (R[b].is_inst()) {
                    ...
                    if (inst.ic_cache != -1 && inst_obj->fields.size() > (size_t)inst.ic_cache) {
                        ...
                        if (offset != -1) {
                            ...
                            inst.ic_cache = offset; // ?§Ïùå Î≤?Î£®ÌîÑ?êÎäî Ï∫êÏãú Hit ?òÎèÑÎ°?Î™ÖÎ†π???ÅÍµ¨ Î≥ÄÏ°?(IC!)
                        } else {
                            R[a] = Value::nil();
                        }
                    }
                    ...
                    if (inst.ic_cache != -1 && inst_obj->fields.size() > (size_t)inst.ic_cache) {
                        ...
                        if (offset != -1) {
                            ...
                            if (rt_classes.count(inst_obj->class_name)) {
                                ...
                                // ?ÑÏû¨ ?ùÏÑ±???∏Ïä§?¥Ïä§?êÎßå ?¨Í∏∞Î•?ÎßûÏ∂îÍ∏??ÑÌï¥ Î¶¨ÏÇ¨?¥Ï¶à Î∞??†Îãπ
            ...
            case JitOp::METHOD_CALL: {
                if (R[b].is_inst()) {
                    ...
                    if (mi) {
                        ...
                        new_frame.registers[0] = R[b]; // self
            ...
            case JitOp::NEW_INSTANCE: {
                ...
                auto ctor = find_method(cls, "?ùÏÑ±??); if (!ctor) ctor = find_method(cls, "init");
                if (ctor) {
                    CallFrame new_frame;
                    ...
                    new_frame.registers[0] = inst_val; // self
